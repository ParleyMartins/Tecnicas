...
public class RoomTeacherReservationDAO extends DAO {
	 ...
	 * @throws ReservaException if an exception related to the reservation is
	 * activated
	 */
	public void insert(TeacherReserveRoom reservation) throws ReservaException,
			SQLException {

		checkReservationNull(reservation);

		if (!this.teacherIsInDB(reservation.getTeacher())) {
			throw new ReservaException(TEACHER_INEXISTENT);
		}

		checkRoom(reservation, true);

		checkReservationDontExists(reservation);
	 ...
	 * activated
	 */
	public void modify(TeacherReserveRoom oldReservation,
			TeacherReserveRoom newReservation) throws ReservaException,
			SQLException {

		checkReservationNull(oldReservation);
		checkReservationNull(newReservation);
		checkReservationExists(oldReservation);
		checkReservationDontExists(newReservation);

		if (!this.teacherIsInDB(newReservation.getTeacher())) {
			throw new ReservaException(TEACHER_INEXISTENT);
		}

		checkRoom(newReservation, true);
		checkRoom(oldReservation, false);

		checkDateTime(newReservation);
		checkDateTime(oldReservation);
	 ...
	 * @throws ReservaException if an exception related to the reservation is
	 * activated
	 * @throws ClienteException if an exception related to the client is
	 * activated
	...
	/**
	 * This searches for all Room Reservations from the database, in the given
	 * date.
	...
	private boolean roomIsInDB(Room room) throws SQLException {
		...
		String query = "SELECT * FROM sala WHERE " + "sala.codigo = \""
	...
	/**
	 * This checks if a given room is in a reservation on a determined day and
	 * time.
	 ...
	 * @param date The String with the wanted date. It must have the XX/XX/XXXX
	 * pattern.
	 * @param time The String with the wanted time. It must have the XX:XX
	 * pattern.
	...
	private boolean roomIsInReservationDB(String date, String time)
		...
		String query = "SELECT * FROM reserva_sala_aluno WHERE " + "data = \""
				+ date + "\" and " + "hora = \"" + time + "\";";
	...
	private boolean dateIsGone(String date) {
		...
		if (Integer.parseInt(now[1]) > Integer.parseInt(dateParts[1])) {
			...
			if (Integer.parseInt(now[1]) == Integer.parseInt(dateParts[1])) {
				...
				dateParts[0] = now[0].substring(0, differenceDates)
						+ dateParts[0];
	...
	private String standardizeDate(String date) {
		...
		for (int i = 0; i < 3; i++) {
			if (i == 0) {
				...
				standardDate += "/"
						+ now[i].substring(0,
								now[i].length() - dateParts[i].length())
						+ dateParts[i];
			}
		}
	...
	private String whereQuery(TeacherReserveRoom reservation) {
		...
		String query = " WHERE " + "id_professor = ( " + selectTeacher
				+ " ) and " + "id_sala = ( " + selectRoom + " ) and "
	...
	private String valuesQuery(TeacherReserveRoom reservation) {
		...
		String query = "( " + selectTeacher + " ), " + "( " + selectRoom
				+ " ), " + "\"" + reservation.getPurpose() + "\", " + "\""
	...
	private String attributesQuery(TeacherReserveRoom reservation) {
		...
		String query = "id_professor = ( " + selectTeacher + " ), "
				+ "id_sala = ( " + selectRoom + " ), " + "finalidade = \""
				+ reservation.getPurpose() + "\", " + "hora = \""
				+ reservation.getTime() + "\", " + "data = \""
				+ reservation.getDate() + "\"";
	...
	/**
	 * This generates a DELETE query from teacher reservations with a given
	 * reservation
	 * @param reservation The RoomReservation to generate the query
	...
	private String deleteFromTeacherQuery(TeacherReserveRoom reservation) {
		...
		String query = "DELETE FROM reserva_sala_professor " + where + " ;";

		return query;
	}

	/**
	 * This generates a DELETE query from student reservations with a given
	 * reservation
	 * @param reservation The RoomReservation to generate the query
	 * @return the DELETE query
	 */
	private String deleteFromStudentQuery(TeacherReserveRoom reservation) {
		...
		String query = "DELETE FROM reserva_sala_aluno WHERE " + "hora = \""
				+ reservation.getTime() + "\" and " + "data = \""
				+ reservation.getDate() + "\" ;";
	 ...
	 */
	private String updateQuery(TeacherReserveRoom oldReservation,
			TeacherReserveRoom newReservation) {

		String attributes = this.attributesQuery(newReservation);
		String where = this.whereQuery(oldReservation);

		String query = "UPDATE reserva_sala_professor SET " + attributes
				+ where + " ;";
